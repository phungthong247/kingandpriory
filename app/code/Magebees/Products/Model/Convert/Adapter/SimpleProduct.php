...
class SimpleProduct {	
	...
	{	
		...
		/*
		if($xyz == 1){
			$currentCategories = $SetProductData->getCategoryIds();			
			$connectioncat = $this->resource->getConnection();
			$catProTable = $this->resource->getTableName('catalog_category_product');
			if(!empty($currentCategories)){
				foreach($currentCategories as $categoryId) {
					$sql = "DELETE FROM ".$catProTable." WHERE category_id=".$categoryId." AND product_id =".$SetProductData->getId();
					$connectioncat->query($sql);
				}
			}
		}	
		*/
		...
		if($mVersion > '2.3.0'){
			if(isset($ProductStockdata['qty']) || $ProductStockdata['qty'] != "" || $ProductStockdata['qty'] != null){
				$this->getSalableQuantityDataBySku->execute($ProcuctData['sku']);
			}
		}
		*/
