...
class Exportproduct extends \Magento\Framework\Model\AbstractModel
{
	...
	public function unparse($page,$store_export_id,$attr_id,$export_for,$type_id,$status_id,$visibility_id,$cat_ids,$fromId,$toId,$fromPrice,$toPrice,$fromStock,$toStock,$export_fields)
    {
		...
		foreach($_product_collection as $product)
		{
			...
			/*
			$sourceDataBySku = $this->helper->getObjectManager()->get('\Magento\InventoryCatalogAdminUi\Model\GetSourceItemsDataBySku');
			$sourcedata = $sourceDataBySku->execute($row['sku']);
			foreach($sourcedata as $data){
				if($data["source_code"] == $row['websites']){
